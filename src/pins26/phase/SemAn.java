...
public class SemAn {
	...
	private static class LValResolver {
		 ...
		 */
		//left value is the address of variable,we have to check if it exists
		//left value can also be izraz for example a[i] where we need the address of a and value of i and i is also left value
		//i by themself has both address and value,but when combined with other things it can either be lvalue or not
		//i^ = 7 is for our lang
		//so we just have to check if its correctly put in right and left?,like 5 = i is not allowed
		private class ResolverVisitor implements AST.FullVisitor<Object, Object> {

			@Override
			public Object visit(final AST.AssignStmt assignStmt, Object args) {
				assignStmt.dstExpr.accept(this,args);
				assignStmt.srcExpr.accept(this,args);
				AST.Expr left = assignStmt.dstExpr;
				if (left instanceof AST.VarExpr || (left instanceof AST.UnExpr unExpr && unExpr.oper == AST.UnExpr.Oper.VALUEAT)) {
	...
	public static void main(final String[] cmdLineArgs) {
		...
		try {
			...
			try (SynAn synAn = new SynAn(cmdLineArgs[0])) {
				...
				// semanticna analiza:
				final AttrAST semanAttrAST = SemAn.analyze(abstrAttrAST);
